...
3. 阅读一个带讲解的标准实现示例，再根据分项提示完成其余 `TODO`；关键不变量可用最小聚焦测试完成 Red/Green。
4. 运行真实示例，先观察输出并修正实现偏差。
5. 最后复制教程给出的完整测试并执行 `bun test`，确认阶段不变量。
...
终端反馈帮助你先建立运行时心智模型。每阶段必须保留一个 `examples/stage-xx-*.ts`，
不能只保留单元测试；先核对示例输出，再进入完整测试。
